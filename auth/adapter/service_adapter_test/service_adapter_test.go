...
func TestUserCheckerAccountExists(t *testing.T) {
	tests := []userTestCase{
		{
			...
			setupMock: func(mockSvc *coreMocks.MockUserService) {
				mockSvc.On("AccountExists", mock.Anything, uint(1)).Return(true, &models.User{Model: gorm.Model{ID: 1}}, nil)
			},
			expectExist: true,
			expectErr:   false,
		},
		{
			...
			userID: 999,
			setupMock: func(mockSvc *coreMocks.MockUserService) {
				mockSvc.On("AccountExists", mock.Anything, uint(999)).
		...
		},
		{
			...
			userID: 1,
			setupMock: func(mockSvc *coreMocks.MockUserService) {
				mockSvc.On("AccountExists", mock.Anything, uint(1)).Return(false, nil, errors.New("database error"))
			},
			expectExist: false,
			expectErr:   true,
		},
	...
	testUserCheckerCases(t, tests, func(mockSvc *coreMocks.MockUserService, userID uint) (bool, error) {
		return adapter.NewUserChecker(mockSvc).AccountExists(context.Background(), userID)
	})
}
...
func TestUserCheckerIsAccountVerified(t *testing.T) {
	tests := []userTestCase{
		{
			...
			userID: 1,
			setupMock: func(mockSvc *coreMocks.MockUserService) {
				mockSvc.On("IsAccountVerified", mock.Anything, uint(1)).Return(true, nil)
			},
			expectExist: true,
			expectErr:   false,
		},
		{
			...
			userID: 2,
			setupMock: func(mockSvc *coreMocks.MockUserService) {
				mockSvc.On("IsAccountVerified", mock.Anything, uint(2)).Return(false, nil)
			},
			expectExist: false,
			expectErr:   false,
		},
		{
			...
			userID: 1,
			setupMock: func(mockSvc *coreMocks.MockUserService) {
				mockSvc.On("IsAccountVerified", mock.Anything, uint(1)).Return(false, errors.New("database error"))
			},
			expectExist: false,
			expectErr:   true,
		},
	...
	testUserCheckerCases(t, tests, func(mockSvc *coreMocks.MockUserService, userID uint) (bool, error) {
		return adapter.NewUserChecker(mockSvc).IsAccountVerified(context.Background(), userID)
	})
}
...
func testAccessCheckerCases(t *testing.T, tests []accessTestCase) {
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			checker := adapter.NewAccessChecker(mockSvc)
	...
	tests := []accessTestCase{
		{
			...
			setupMock: func(mockSvc *coreMocks.MockAccessService) {
				mockSvc.On("CheckAccess", mock.Anything, uint(1), "example.com", "/api/data", "GET").
					Return(true, nil)
			},
			expectAccess: true,
			expectErr:    false,
		},
		{
			...
			method: "POST",
			setupMock: func(mockSvc *coreMocks.MockAccessService) {
				mockSvc.On("CheckAccess", mock.Anything, uint(2), "example.com", "/admin", "POST").
					Return(false, nil)
			},
			expectAccess: false,
			expectErr:    false,
...
func TestNewUserCheckerFromCore(t *testing.T) {
	...
	// Set up expectations
	mockUserSvc.On("AccountExists", mock.Anything, uint(1)).
		Return(true, &models.User{Model: gorm.Model{ID: 1}}, nil)
	mockUserSvc.On("IsAccountVerified", mock.Anything, uint(1)).Return(true, nil)
...
func TestNewAccessCheckerFromCore(t *testing.T) {
	...
	// Set up expectations
	mockAccessSvc.On("CheckAccess", mock.Anything, uint(1), "example.com", "/api", "GET").
		Return(true, nil)
	...
	// Create the checker from the context
	checker := adapter.NewAccessCheckerFromCore(ctx)
